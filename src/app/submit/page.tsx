...
export default function SubmitPage() {
  ...
  const goTo3 = async () => {
    ...
    // Manually gate agency name before trigger so the error is visible
  ...
  const onSubmit = async (data: ReviewInput) => {
    ...
    // redirect() in the action navigates away; if we're still here it was a soft error
  };
          ...
          {step === 3 && (
            <>
              ...
              <SectionCard title="Quick questions">
                ...
                />
                {/* Only show deposit question if tenancy has ended */}
                {tenancyEnd && (
                  <Controller
